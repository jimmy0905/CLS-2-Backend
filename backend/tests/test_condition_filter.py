...
class SurveyFilterDateRangeTests(unittest.TestCase):
    def test_parses_utc_timestamp_boundaries_as_naive_utc_datetimes(self) -> None:
        ...
        self.assertEqual(conditions[1].right.value, datetime(2026, 8, 3, 16, 0))
        self.assertEqual(conditions[2].right.value, datetime(2026, 8, 4, 16, 0))

    def test_converts_offset_aware_timestamp_to_naive_utc_datetime(self) -> None:
        conditions = build_survey_filter_conditions(
